...
lean_markers = ["LEAN_X", "LEAN_Y", "LEAN_Z", "TORSO_LEAN"]
...
head_markers = ["HEAD_X", "HEAD_Y", "HEAD_Z", "HEAD_DIRECTION", "HEAD_PUSH"]  # to be used in head pose estimation
...
for ff in eachfile:
    ...
    # they are used in head and torso pose estimation
    # camera matrix
    ...
    with mp_holistic.Holistic(static_image_mode=False,
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                # add to pose rows
                lean_row = [np.round(lean_x, 6), np.round(lean_y, 6), np.round(lean_z, 6), torso_text]
                pose_row = pose_row + lean_row

                # draw on the video
                cv2.putText(image, "Torso: " + torso_text, (1600, 300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0),
                            2)
                cv2.putText(image, "x: " + str(np.round(lean_x, 2)), (1600, 350),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                cv2.putText(image, "y: " + str(np.round(lean_y, 2)), (1600, 400),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
                cv2.putText(image, "z: " + str(np.round(lean_z, 2)), (1600, 450),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
            else:
                    pose_row = list(np.array([np.nan] * 34 * 4).flatten())
            ...
            # below will be used in head pose estimation
            face_2d = []
            face_3d = []
            face_row = []
            if results.face_landmarks:
                ...
                face_row = [np.round(head_x, 6), np.round(head_y, 6), np.round(head_z, 6), text, text2]

                # to draw nose point direction on the image
                ...
                face_row = list(np.array([np.nan] * 5).flatten())
            ...
            timee = timee + (1000 / samplerate)
            print(f"FPS: {np.round(fps2, 2)}, at {timee / 1000} seconds")
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # to exit elegantly if cv2.imshow is uncommented
            break
    ...
    # interpolation of NA can be integrated into this